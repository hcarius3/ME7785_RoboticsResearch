...
def initialize_model(model_path=None):
    ...
    if model_path is None:
        raise ValueError("Model path must be provided to initialize the model.")
...
def predict(model, image):
    ...
    image = cv2.resize(image, (25, 33))

    # Make sure it's in color (BGR). If already BGR, this won't change anything
    if len(image.shape) == 2:
        image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)

    # Flatten and normalize the image
    sample = image.flatten().astype(np.float32).reshape(1, -1) / 255.0  # Normalize to [0, 1]

    # Predict the class using the scikit-learn model
